...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double up = 1./(1 + exp(-2*_beta*_J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) -2*_beta*_H));
    if(up > _rnd.Rannyu()) {
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    } else if( property == "E_STEPS" ){
      input >> _equilibration_steps;
      coutf << "E_STEPS= " << _equilibration_steps << endl;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutgr << "# BLOCK:     DISTANCE:     ACTUAL_AVE_GOFR:" << endl;
        coutgr.close();
        ofstream coutgr_fin("../OUTPUT/gofr_fin.dat");
        coutgr_fin << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr_fin.close();
        input>>_n_bins;
        _nprop+=_n_bins;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
          if(dr < _halfside.min()){
            bin = _index_gofr + int(dr/_bin_size);
            _measurement(bin) += 2;
          } 
        }
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          if(_measure_pressure) virial += 1.0/pow(dr,12) - 0.5*(1.0/pow(dr,6)); // PRESSURE 
        }
      }
    }
  }
  ...
  if (_measure_pressure){
    if (_sim_type == 0){
      ...
      _measurement(_index_pressure) = _ptail + _rho*_temp + (48/(3*_volume))*virial;
    }
  } 
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet) {
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
   if (_measure_cv) {
    double s_i, s_j;
    double tenergy_squared = 0.;
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        tenergy_squared += pow(- _J * s_i * s_j - 0.5 * _H * (s_i + s_j), 2);
      }
      tenergy_squared /= double(_npart);
      _measurement(_index_cv) = pow(_beta, 2)*(tenergy_squared - pow(tenergy_temp, 2));
   }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
   if (_measure_chi) {
    for (int i = 0; i < _npart; i++){
      _measurement(_index_chi) += _particle(i).getspin();
    }
    _measurement(_index_chi) = _beta*pow(_measurement(_index_chi),2);
    _measurement(_index_chi)/=_npart;
   }
  // GOFR
   if (_measure_gofr){
    for (int i = 0; i < _n_bins; i++){
      double delta_V = (4.0*M_PI/3.0)*(pow(_bin_size*i + _bin_size, 3) - pow(_bin_size*i, 3));
      _measurement(_index_gofr + i) /= (_rho*_npart*delta_V); 
    }
   }
...
int System :: get_equilibration_steps(){
  return _equilibration_steps;
}
