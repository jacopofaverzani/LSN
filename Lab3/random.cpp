...
double Random :: exp(double rate){
   // This function generates a random number from a exponential distribution with given rate
   double x = Rannyu();
   return - (1 / rate) * log(1 - x); 
}

double Random :: Lorentz(double loc, double width){
   // This function generates a random number from a Cauchy-Lorentz distribution with given location and width
   double x = Rannyu(0.1, 0.90);
   return width * tan(M_PI * (x - 0.5)) + loc;
}
