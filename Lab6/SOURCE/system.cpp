...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure) pressure_temp += 1.0/pow(dr,12) - 0.5*(1.0/pow(dr,6)); // PRESSURE 
        }
      }
    }
  }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) _measurement(_index_pressure) = _rho*_measurement(_index_temp) + (48/(3*_volume))*pressure_temp;
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet) {
    for (int i = 0; i < _npart; i++){
